       ...
       #     user_good_list = user_good
        ...
        for user_good_key in user_good :
            ...
            if (user_dic.has_key(user) is True) and (good_dic.has_key(good) is True) :
                ...
                user_feature = GenerateOneUserFeature(user_dic[user],start_date,cut_date + 1)   #生成商品特征
                if user_feature == False:    #用户在选定时间内没有操作过商品，所以忽略后续操作
                    continue
             #   good_feature = GenerateOneGoodFeature(good_dic[good],start_date,cut_date + 1)   #生成用户特征
                ...
                feature = []
                if out_put_type == 0 :
                    result = JudgeResult(user_good_key,use_item_result,cut_date+1,cut_date + 2) #result由cut_date及其后两天决定
                   # if result == True:
