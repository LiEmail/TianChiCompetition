...
def GenerateFeature(out_put_type) :
        ...
        for row in reader:
            ...
            if (out_put_type  == 1 and b_time < test_time_stamp) or \
               (out_put_type == 0 and b_time < cut_time_stamp ):
                ...
                one_user[behavior_type] = one_user[behavior_type] + 1   #更新行为
                if b_time - cut_time_stamp <= days_feature[0] :
                    user_good[use_good_id][days_feature[0]] = user_good[use_good_id][days_feature[0]] + 1
                    user_good[use_good_id][days_feature[1]] = user_good[use_good_id][days_feature[1]] + 1
                    user_good[use_good_id][days_feature[2]] = user_good[use_good_id][days_feature[2]] + 1
                if b_time - cut_time_stamp <= days_feature[1] :
                    user_good[use_good_id][days_feature[0]] = user_good[use_good_id][days_feature[0]] + 1
                    user_good[use_good_id][days_feature[1]] = user_good[use_good_id][days_feature[1]] + 1
                    user_good[use_good_id][days_feature[2]] = user_good[use_good_id][days_feature[2]] + 1
                if b_time - cut_time_stamp <= days_feature[2] :
                    user_good[use_good_id][days_feature[0]] = user_good[use_good_id][days_feature[0]] + 1
                    user_good[use_good_id][days_feature[1]] = user_good[use_good_id][days_feature[1]] + 1
                    user_good[use_good_id][days_feature[2]] = user_good[use_good_id][days_feature[2]] + 1
                '''
    ...
    count = 0;
    for key in user_good:
        array = key.split('-')
        user = array[0]
        good = array[1]
        if user_dic.has_key(user) == True:
            if good_dic.has_key(good) == True:
                target = 0
                if user in use_item_result:
                    target = 1
                #####2015-04-15 chenlin
                user_good_item = user_good[key]
                                ##################
                ...
                user_good_f1 = user_good[key][days_feature[0]]
                user_good_f2 = user_good[key][days_feature[1]]
                user_good_f3 = user_good[key][days_feature[2]]
                #判断 一些刷单用户，取消噪点
                ...
                if out_put_type == 0 :
                    if target == 0 and clearNagetiveSample(user_good_item) == True:
                        continue
                    if target == 0 and count == 7 :
                        writer.writerow([target, good_f1, good_f2, good_f3, good_f4, user_f1, user_f2, user_good_f1, user_good_f2, user_good_f3])
                        count = 0
                    ...
                    writer.writerow([user, good, good_f1, good_f2, good_f3, good_f4, user_f1, user_f2, user_good_f1, user_good_f2, user_good_f3])
    csvfile.close()
    print 'generate over'
    return
